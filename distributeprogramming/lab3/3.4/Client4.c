...
int sendName_xdr(int sfd,char *filename,char *ptr,FILE *stream_socket_w,message *m);
int readControl_xdr(int sfd,FILE *stream_socket_w,message *m);
int readSizeTime_xdr(int sfd,int *fileSize,FILE *stream_socket_w,message *m);
char *prog_name;
//int daemon_proc=0;
...
return 0;}
	...
	memset(&m,0,sizeof(message));
	//calloc(&n,sizeof(message));
       if(sendName_xdr(sfd,filename,ptr1,stream_socket_w,&m)!=0)
       	break;
         //---------------send name----------------
	...
	memset(&m_out,0,sizeof(message));
	//if (!xdr_tagtype (&xdrs_r, &m_out.tag))///why stuck here?
	//	printf("error in receiving\n");
	xdr_message(&xdrs_r,&m_out);
    		...
    		case OK:
    			return_value =1;
    			printf("received ok\n!");
    			fileSize = m_out.message_u.fdata.contents.contents_len;
    			printf("the filesize is %u\n",fileSize);
    			u_int time =m_out.message_u.fdata.last_mod_time;
...
int f=fwrite(m_out.message_u.fdata.contents.contents_val,1,fileSize,fp);
    ...
    trace(err_msg("(%s)---download finished!",prog_name));
...
int sendName_xdr(int sfd,char *filename,char *ptr1,FILE *stream_socket_w,message *m){
	
	XDR xdrs_w;
	int i = 0;
	//memset(m,0,sizeof(message));
	//m = calloc(m,sizeof(message));
...
int readControl_xdr(int sfd,FILE *stream_socket_r,message *m){
